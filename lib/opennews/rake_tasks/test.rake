...
namespace :test do
    ...
    begin
      HTMLProofer.check_directory(
        "./_site",
        {
          ...
          log_level: :error,
          ignore_files: cfg.html_proofer_ignore_files,
          ignore_urls: cfg.html_proofer_ignore_urls,
        },
      ).run
    ensure
    ...
    Dir
      ...
      .each do |file|
        ...
        if content =~ /\A---\s*\n(.*?)\n---\s*\n/m
          ...
          errors << "#{file}: No YAML front matter found"
        end
      end
  ...
  desc "Check for common accessibility issues"
  task :a11y do
    issues = []
  ...
  desc "Check for performance issues"
  task :performance do
    warnings = []
